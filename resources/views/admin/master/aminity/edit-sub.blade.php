                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="form-validation">
                                    <form class="form-valide" method="post" action="{{ route('admin.master.update.sub.aminities') }}">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ encrypt($subAminities->id) }}">
                                            ...
                                            <div class="col-lg-6">
                                                <select class="form-control" name="main_aminity_name">
                                                    ...
                                                    @foreach ($mainAminities as $aminities)
                                                        <option value="{{ $aminities->id }}" @selected($aminities->id ==$subAminities->main_aminities_id)>{{ $aminities->aminity_name }}</option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-6">
                                                <input type="text" class="form-control" id="val-username" name="name" placeholder="Enter a aminity.." value="{{ old('sub_aminity_name')??$subAminities->name }}">
                                                @error('name')
                                                 <span class="text-danger">{{ $message }}</span>   
                                                @enderror
                                            </div>
